...
def report(args, unreconciled, reconciled, explanations, column_types):
    ...
    reconciled = reconciled.astype(str)
    unreconciled = unreconciled.astype(str)

    # Convert links into anchor elements (element-wise)
    reconciled = reconciled.apply(lambda col: col.map(create_link))
    unreconciled = unreconciled.apply(lambda col: col.map(create_link))
